<template>
  <div class="blog-container">
    ...
    <el-table :data="data" style="width: 100%" border>
      ...
      <el-table-column prop="scanNumber" label="浏览数">
        <template slot-scope="scope">{{ scope.row.scanNumber }}</template>
      </el-table-column>
      <el-table-column prop="commentNumber" label="评论量">
      ...
      <el-table-column prop="createDate" label="创建日期" width="230">
    ...
    fetchData() {
      findBlog(this.currentPage, this.eachPage).then(({ data }) => {
        ...
        if (this.currentPage > this.totalPage) {
          //设想加入删除了条数，总页数变少了，但是你的分页还在未删除的那页
